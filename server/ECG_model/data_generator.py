...
class Siamese_Loader:
    ...
    def __init__(self, path, data_subsets=["train", "val"]):
        ...
        self.file_set = [
            ...
            for file in (
                glob.glob(
                    path + "ecg-id-database-filter/Person_{:02d}/rec_*.png".format(num)
                )
    ...
    def get_batch(self, batch_size, s="train"):
        ...
        for i in range(batch_size):
            ...
            pair_x, pair_y = np.random.choice(
                self.file_set[idx_1 - 1], 2, replace=False
            ).tolist()
    ...
    def make_oneshot_task(self, N, s="val"):
        ...
        pair_x = np.random.choice(
            self.file_set[true_idx - 1], N + 1, replace=False
        ).tolist()
        ...
        for i in range(1, N):
            pair_y = np.random.choice(
                self.file_set[false_idx[i - 1]], 1, replace=False
            )[0]
            support_set[i] = (
                np.array(Image.open(pair_y).resize((h, w)))[:, :, 0:1] / 255
            )
